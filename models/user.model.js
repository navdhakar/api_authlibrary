...
function validateUser(user) {
  const schema = Joi.object({
    ...
    college_year: Joi.string().min(3).max(255).required(),
    college_branch: Joi.string().min(3).max(255).required(),
    college_name: Joi.string().min(3).max(255).required(),
    ...
    github: Joi.string().min(3).max(255).required(),
